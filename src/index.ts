...
const GET_PROJECTS_WITH_TASKS_TOOL: Tool = {
...
const RENAME_PROJECT_TOOL: Tool = {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    if (name === "todoist_create_task") {
      ...
      let projectId: string | undefined = (args as any).project_id;
      if (!projectId && (args as any).project_name) {
        projectId = await resolveProjectIdByName((args as any).project_name);
      }
      let sectionId: string | undefined = (args as any).section_id;
      if (!sectionId && (args as any).section_name && projectId) {
        sectionId = await resolveSectionIdByName(projectId, (args as any).section_name);
      }

      // Resolve parent task by name if needed
      let parentId: string | undefined = (args as any).parent_task_id;
      if (!parentId && (args as any).parent_task_name) {
        const { matches } = await disambiguateByName((args as any).parent_task_name, (args as any).project_name);
    ...
    if (name === "todoist_get_tasks") {
      ...
      if (args.project_name && !args.project_id) {
        ...
        if (resolved) apiParams.projectId = resolved;
      }
      if (args.project_id) {
      ...
      if (args.label_name) {
        ...
        if (labelId) apiParams.labelIds = [labelId];
      }
      if (args.filter) {
    ...
    if (name === "todoist_get_projects_with_tasks") {
      ...
      const includeEmpty: boolean = !!a?.include_empty;
      const limitPer: number = (typeof a?.limit_per_project === 'number' && a.limit_per_project > 0) ? a.limit_per_project : 10;
      ...
      const results: Array<{ project: { id: string, name: string }, tasks: any[] }> = [];
      for (const p of projects) {
        const params: any = { projectId: p.id };
        if (a?.label_name) params.label = a.label_name;
        if (a?.filter) params.filter = a.filter;
        let tasks = await todoistClient.getTasks(params);
        if (a?.priority) tasks = tasks.filter(t => t.priority === a.priority);
        if (limitPer) tasks = tasks.slice(0, limitPer);
      ...
      const text = results.map(r => {
        ...
        if (r.tasks.length === 0) return `${header}\n  (no open tasks)`;
    ...
    if (name === "todoist_get_completed_tasks") {
      ...
      const headers = { Authorization: `Bearer ${TODOIST_API_TOKEN}` } as Record<string, string>;
      const collected: Array<{ id?: string; task_id?: string; task?: any; content?: string; completed_at?: string }> = [];
      ...
      for (let i = 0; i < 50; i++) {
        ...
        if (args.until) url.searchParams.set('until', args.until);
    ...
    if (name === "todoist_update_task") {
      ...
      let targetId: string | undefined = args.task_id;
    ...
    if (name === "todoist_list_sections") {
      const a = request.params.arguments as any;
      let projectId: string | undefined = a?.project_id;
    ...
    if (name === "todoist_create_section") {
      const a = request.params.arguments as any;
      if (!a || typeof a.name !== 'string') throw new Error('Invalid arguments for todoist_create_section');
      let projectId: string | undefined = a.project_id;
      if (!projectId && typeof a.project_name === 'string') {
        projectId = await resolveProjectIdByName(a.project_name);
      }
      if (!projectId) throw new Error('Provide project_id or a valid project_name');
    ...
    if (name === "todoist_create_project") {
      const a = request.params.arguments as any;
      if (!a || typeof a.name !== 'string') throw new Error('Invalid arguments for todoist_create_project');
      let parentId: string | undefined = a.parent_project_id;
    ...
    if (name === "todoist_rename_project") {
      const a = request.params.arguments as any;
      if (!a || typeof a.new_name !== 'string') throw new Error('Invalid arguments for todoist_rename_project');
      let projectId: string | undefined = a.project_id;
      if (!projectId && typeof a.project_name === 'string') {
        projectId = await resolveProjectIdByName(a.project_name);
      }
